# Web App Template (tRPC + OAuth + Database)
...
2. Use the returned storage path directly in your code: `<img src="/r2-storage/image_a1b2c3d4.png" />`
...
Use the preconfigured storage helpers in `server/storage.ts`. Credentials are injected from the platform (no manual setup required). Files are stored securely and served via the built-in `/r2-storage/` path — no manual URL management needed.
...
// url = "/r2-storage/{key}" — use directly in frontend code
...
- The returned `url` (e.g. `/r2-storage/...`) is automatically served via signed redirect — no manual URL signing needed.
...
```ts
{
  ...
  "devDependencies": {
    ...
    "typescript": "5.9.3",
    "vite": "^7.1.7",
    "vite-plugin-OAuth-runtime": "^0.0.57",
    "vitest": "^2.1.4"
  },
...
const url = `https://${projectName}.OAuth.space/callback`;
