...
import { BookOpen, Home, Sun, Search, CalendarCheck2 } from "lucide-react";
import { RosaryIcon } from "@/components/RosaryIcon";

const mobileLinks = [
  { href: "/dashboard", label: "Início", icon: Home },
  { action: "search", label: "Busca", icon: Search },
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 lg:hidden border-t border-[oklch(0.75_0.12_75/0.25)] bg-[oklch(0.12_0.03_260/0.96)] text-white shadow-[0_-12px_30px_oklch(0.08_0.02_260/0.22)] backdrop-blur-xl pb-[env(safe-area-inset-bottom)]">
        <div className="grid grid-cols-6 px-2 py-1.5 gap-0.5">
          {mobileLinks.map((item) => {
            const isNavLink = "href" in item;
            const isActive = isNavLink && isActiveRoute(location, item.href);
            const Icon = item.icon;
            const key = isNavLink ? item.href : item.action;
            ...
            return (
              <button
                ...
                className="flex h-[3.75rem] w-full flex-col items-center justify-center gap-1 rounded-xl transition-all text-[oklch(0.78_0.03_260)] hover:text-white"
              >
                <Icon size={20} />
